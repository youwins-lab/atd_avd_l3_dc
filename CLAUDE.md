...
**Host endpoint topology**: `s{n}-host1` is dual-homed via MLAG port-channel (LACP active, trunk VLAN 10/20)
to `s{n}-leaf1`/`s{n}-leaf2`; `s{n}-host2` to `s{n}-leaf3`/`s{n}-leaf4`. Each host has local `vlan 10`/`vlan 20`
entries (required — `switchport trunk allowed vlan` alone does not create the local VLAN on EOS) plus
Vlan10/Vlan20 SVIs with a per-host test IP, used to verify the EVPN-VXLAN stretch by pinging across DCs
(leaf anycast gateways are `10.10.10.1`/`10.20.20.1` in both DCs).
